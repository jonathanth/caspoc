...
#' @importFrom magrittr %>%
#' @export
CASPOC <- function (X, Y, ncomp = 1, numRepeats = 11, numFolds = 10, keepX_options = NULL, keepY_options = NULL, fixX = NULL, fixY = NULL, base_seed = 1, manual_folds = NULL, sign_flipping = TRUE, sign_flipping_method = "PCA", binary = FALSE) {
  ...
  # ============================================================
  # Detect sign orientation of Repeat/Fold loading vectors
  #
  # method = "pca"
  #   Uses the first principal component as the common axis.
  #
  # method = "cosine"
  #   Uses a representative vector followed by a consensus
  #   loading vector.
  #
  # Returns the same output structure for either method.
  # ============================================================
  
  detect_loading_signs <- function(
    loading_matrix,
    repeat_fold_df,
    method = "cosine"
  ) {
    ...
    if (!method %in% c("pca", "cosine")) {
      stop(
        "'method' must be either 'pca' or 'cosine'."
      )
    }
    
    
    # ----------------------------------------------------------
    # Check and normalize loading vectors
    # ----------------------------------------------------------
    
    loading_norms <- sqrt(
      rowSums(loading_matrix^2)
    )
    ...
    valid_rows <-
      ...
      is.finite(loading_norms) &
      loading_norms >
      sqrt(.Machine$double.eps)
    
    
    evaluation <- repeat_fold_df %>%
      dplyr::mutate(
        valid_loading_vector = valid_rows,
        alignment_score = NA_real_
      )
    
    
    # Need at least two usable loading vectors
    if (sum(valid_rows) < 2) {
    ...
    # Unit-normalized loading vectors
    normalized_loadings <- sweep(
      loading_matrix[
        valid_rows,
        ,
        drop = FALSE
    ...
    if (method == "pca") {
      ...
      # PC1 direction is a unit vector
      direction_vector <-
        pca$rotation[, 1]
      
      
      # Using normalized loadings here gives a score from
      # -1 to +1.
      #
      # Importantly, its SIGN is exactly the same as the
      # ordinary PC1 score used in the original CASPOC method.
      alignment_score <- as.vector(
        normalized_loadings %*%
          direction_vector
      )
    ...
    if (method == "cosine") {
      ...
      # --------------------------------------------------------
      # Choose representative loading vector
      # --------------------------------------------------------
      
      if (nrow(similarity_matrix) == 2) {
        
        reference_local_index <- 1L
      ...
      # --------------------------------------------------------
      # Provisionally orient everything towards reference
      # --------------------------------------------------------
    ...
    # ----------------------------------------------------------
    # Common sign decision for BOTH methods
    #
    # score < 0  -> flip
    # score > 0  -> keep
    # ----------------------------------------------------------
  ...
  # Validate sign-flipping method
  if (sign_flipping == TRUE) {
    
    sign_flipping_method <- tolower(sign_flipping_method)
    
    if (!sign_flipping_method %in% c("pca", "cosine")) {
      stop(
        "Error: 'sign_flipping_method' must be either 'pca' or 'cosine'."
      )
    }
    
    method_label <- switch(
      sign_flipping_method,
      "pca" = "PCA",
  ...
  if (sign_flipping == TRUE) {
    ...
    cat(
      sprintf(
        "Output will be checked for sign flipping ambiguity, and will attempt to automatically align the signs of the output using %s.\n\n",
        method_label
      )
    )
    ...
    cat(
      "Note: Output will not be checked for sign flipping ambiguity. Please verify manually if sign flipping occurred by viewing the loadings.\n\n"
    )
  }
    ...
    for (x in keepX_options) {
      for (y in keepY_options) {
        ...
        for (i in 1:numFolds) {
          ...
          # Explicitly preserve outcome names
          colnames(trainY) <- colnames(Y)
          colnames(tuneY)  <- colnames(Y)
          colnames(testY)  <- colnames(Y)
  ...
  if (sign_flipping == TRUE) {
    ...
    cat(
      sprintf(
        "\nChecking for sign ambiguity and resolving output using %s.\n",
        method_label
      )
    )
    
    # Significant combinations to inspect for sign flipping
    # Keep the existing CASPOC workflow: only combinations whose median-repeat
    # tuning correlation is positive and significant are evaluated further.
    sig_combos <- results_tune_df %>%
      ...
      # Only use valid tuning results
      dplyr::filter(
        is.finite(Score_cor),
        is.finite(Score_cor_p)
      ) %>%
      
      # Calculate median tuning correlation
      dplyr::mutate(
        corr_median = median(
          Score_cor,
          na.rm = TRUE
      ...
      # Select the actual repeat closest to that median
      dplyr::slice_min(
        order_by = abs(
          Score_cor - corr_median
        ),
        n = 1,
        with_ties = FALSE
      ) %>%
      
      # Your significance criterion
      dplyr::filter(
        Score_cor_p <= 0.05,
        Score_cor > 0
      ) %>%
    ...
    # Summary: one row per significant KeepX/KeepY/Component combination
    ...
    for (i in seq_len(nrow(sig_combos))) {
      ...
      # ------------------------------------------------------------
      # Construct joint X + Y loading vector
      # ------------------------------------------------------------
      
      x_loadings_i <- full_train_loadingsX %>%
        dplyr::filter(
          KeepX == keepx_i,
          KeepY == keepy_i
        ) %>%
      ...
      y_loadings_i <- full_train_loadingsY %>%
        dplyr::filter(
          KeepX == keepx_i,
          KeepY == keepy_i
        ) %>%
      ...
      flip_summary_list[[i]] <-
        tibble::tibble(
          ...
          mean_abs_alignment =
            sign_result$mean_abs_alignment,
          
          min_abs_alignment =
            sign_result$min_abs_alignment,
          
          reference_repeat =
            sign_result$reference_repeat,
          
          reference_fold =
            sign_result$reference_fold,
          
          alignment_status =
            sign_result$alignment_status
        )
    ...
    n_sig_combos_flipped <- if (nrow(flip_summary_df) > 0) {
      sum(
        flip_summary_df$flipped_any,
        na.rm = TRUE
      )
    } else {
      0L
    ...
    cat(
      sprintf(
        "Number of significant combinations checked: %d.\n",
        n_sig_combos
      )
    )
    ...
    cat(
      sprintf(
        "Total number of flipped Repeat/Fold cases: %d.\n",
        total_n_flips
      )
    )
    ...
    if (n_sig_combos_flipped > 0) {
      ...
      cat(
        sprintf(
          "\nSign flipping has been detected using %s and the output of the significant combinations has been adjusted accordingly.\n",
          method_label
        ),
        "A detailed sign flip summary and log have been output in flip_summary_df and flip_log_df."
      )
      ...
      cat(
        sprintf(
          "\nNo sign flips detected using %s.",
          method_label
        )
      )
    }
  ...
  return(
    list(
      results_tune_df = results_tune_df,
